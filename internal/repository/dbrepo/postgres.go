...
// AllReservation returns a slice of all reservations ordered by start date
...
// AllNewReservation returns a slice of new reservations ordered by start date
...
// UpdateProcessedForReservation updates processed for a new reservation by id
...
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	_, err := m.DB.ExecContext(ctx, query, startDate, startDate.AddDate(0, 0, 0), id, 2, time.Now(), time.Now())
	if err != nil {
		log.Println(err)
		return err
	}
